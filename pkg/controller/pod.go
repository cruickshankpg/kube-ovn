...
func (c *Controller) getPodDefaultSubnet(pod *v1.Pod) (*kubeovnv1.Subnet, error) {
	...
	for subnetName := range strings.SplitSeq(subnetNames, ",") {
		...
		switch {
		case ipFamily == util.IPFamilyIPv4 && subnet.Spec.Protocol == kubeovnv1.ProtocolDual:
		...
		case ipFamily == util.IPFamilyIPv6 && subnet.Spec.Protocol == kubeovnv1.ProtocolDual:
